...
\href{https://github.com/michal-h21/tex4ht-booksite}{GitHub repository page} (see figure~\ref{fig:template}).
...
\chapter{Styling the Output}

Most of the colors used in the template are defined through CSS variables,  
which can be customized directly in the configuration file.


An example how you could  change the output shows the following CSS rule. Add
the following text to your \texttt{config.cfg} configuration file:
...
The \verb|\Css| command allows you to insert raw CSS rules directly
into the generated CSS file. The content is not read verbatim, so we need to escape special characters such as \texttt{\#} 
or \texttt{\%}. Otherwise, they would cause compilation errors. In this case, we need to escape the hash sign
used in hexadecimal color codes.

Css variables are prefixed with \texttt{--} and can be set in a selector that is active in a whole document, such as \texttt{body}.
In this example, we set the background and text color of the
navigation menu. 

\begin{figure}[htbp]
  \includegraphics[width=\textwidth,alt={Image of customized page design}]{img/book-changecolor.png}
  \caption{Customized page design with modified colors}
\end{figure}
...
For each of these properties, you may define both the foreground and background colors  
by appending \texttt{-color} and \texttt{-background} to the property name.
